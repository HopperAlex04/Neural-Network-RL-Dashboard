...
def test_trainee_win_rate_vs_gapmaximizer_per_round(tmp_path):
    """Trainee win rate vs GapMaximizer: one point per round, x=round, y=that round's final win rate."""
    # Round 0 (trainee_first): last record p1_wins=1, p2_wins=1 -> trainee (P1) 1 win / 2 games = 0.5
    ...
    # Round 1 (trainee_second): last record p1_wins=0, p2_wins=2 -> trainee (P2) 2 wins / 2 games = 1.0
    ...
    assert x == [0.0, 1.0], "x should be round index (0, 1)"
    assert len(y) == 2
    assert y[0] == 0.5  # Round 0 final: 1 win / 2 games
    assert y[1] == 1.0  # Round 1 final: 2 wins / 2 games
...
def test_trainee_win_rate_vs_gapmaximizer_averages_both_seats(tmp_path):
    """When a round has both trainee_first and trainee_second files, y is the average of the two win rates."""
    # Round 0: trainee_first 1/2 = 0.5, trainee_second 2/2 = 1.0 -> avg = 0.75
    (tmp_path / "metrics_round_0_vs_gapmaximizer_trainee_first.jsonl").write_text(
        '{"episode": 1, "p1_wins": 1, "p2_wins": 1}\n', encoding="utf-8"
    )
    (tmp_path / "metrics_round_0_vs_gapmaximizer_trainee_second.jsonl").write_text(
        '{"episode": 1, "p1_wins": 0, "p2_wins": 2}\n', encoding="utf-8"
    )
    reader = MetricsReader(stat_keys=[TRANEE_WIN_RATE_VS_GAPMAXIMIZER_KEY])
    reader.add_path(tmp_path)
    ...
    x, y = reader.get_series(TRANEE_WIN_RATE_VS_GAPMAXIMIZER_KEY)
    assert x == [0.0]
    assert y[0] == 0.75  # (0.5 + 1.0) / 2


def test_trainee_win_rate_ignores_selfplay(tmp_path):
    """trainee_win_rate_vs_gapmaximizer uses only GapMaximizer files; selfplay files ignored."""
    (tmp_path / "metrics_round_0_selfplay.jsonl").write_text(
